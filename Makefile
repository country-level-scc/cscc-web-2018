...
scratch/co2e.zip: scratch
	curl -o $@ 'http://api.worldbank.org/v2/en/indicator/EN.ATM.CO2E.KT?downloadformat=csv'

scratch/LATEST.EN.ATM.CO2E.csv: scratch/co2e.zip
	unzip -d $(SCRATCH_DIR) -u '$(SCRATCH_DIR)/co2e.zip'
	mv scratch/API_EN*.csv $@
	touch $@

sourcedata/co2e-with-headers.csv: scratch/LATEST.EN.ATM.CO2E.csv
	sed -e 1,4d < $(SCRATCH_DIR)/LATEST.EN.ATM.CO2E.csv > $@

sourcedata/co2e-latest.csv: sourcedata/co2e-with-headers.csv
	python scripts/clean-emissions.py sourcedata/co2e-with-headers.csv > sourcedata/co2e-latest.csv
